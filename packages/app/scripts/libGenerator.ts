...
const DTS_FILE_NAME = "opstrace.d.ts";
...
function getVirtualDtsPath(resolvedImportPath: string) {
  return /\/node_modules\//.test(resolvedImportPath)
    ...
    : resolvedImportPath.replace(/^.*src\//, "@opstrace/").replace(/^@opstrace\/client\//, "@node_modules/_internal/");
}
...
`;
  ...
  ts.sys.writeFile(path.resolve(process.cwd(), `src/${DTS_FILE_NAME}`), content);
}
...
async function generateDts(sourceFiles: string[]) {
  ...
  console.log(`✨ dts generated, took ${
    Date.now() - start
  }ms`);
...
export async function bundle(files: string[]) {
  ...
  files.map(file => 
  console.log(
    `✨ bundled: @opstrace/${removeExtension(file.replace(/^.*src\/lib\//, ""))} took ${
      Date.now() - start
    }ms`
  ));
}
...
async function compileLibrary() {
  ...
  .sync(path.resolve(process.cwd(), "src/lib/**/*.ts"))
...
function watch() {
  const watcher = chokidar.watch(path.resolve(process.cwd(), "src/**/*{.ts,.tsx}"), {
    persistent: true,
