  ...
  test("should not subscribe for actions twice", async () => {
    ...
    await expectSaga(subscribeSaga)
      .provide([
        // Mock part
        [
          matchers.call.fn(fileSubscriptionEventChannel),
          eventChannel(() => () => {})
        ]
      ])
      .take(actions.subscribe)
