...
const isDuration = {
  ...
  message:
    ...
    "${path} is not a valid duration. Must be [number][s|m|h|d|w], e.g. 2d for 2 days.",
  test: (value: any) => /^\d+[s|m|h|d|w]{1}$/.test(value || "")
};

export const cortexLimitsSchema = yup.object({
  ingestion_rate: yup.number().integer().meta({
    description: "Ingestion rate limit in samples per second for this tenant."
  }),
  ingestion_burst_size: yup.number().integer().meta({
    description:
  ...
  ha_max_clusters: yup.number().integer().meta({
    description:
  ...
  max_label_value_length: yup.number().integer().meta({
    description:
  ...
  max_metadata_length: yup.number().integer().meta({
    description:
  ...
  ingestion_tenant_shard_size: yup.number().integer().meta({
    description:
  ...
  max_query_length: yup
    ...
    .test(isDuration),
  max_query_parallelism: yup.number().integer().meta({
    description:
  ...
  max_cache_freshness: yup
    ...
    .test(isDuration),
  max_queriers_per_tenant: yup.number().integer().meta({
    description:
  ...
  ruler_evaluation_delay_duration: yup
    ...
    .test(isDuration),
  ruler_tenant_shard_size: yup.number().integer().meta({
    description:
      "This tenant's shard size. A value of 0 disables shuffle sharding for the Ruler in this tenant."
  }),
  ruler_max_rules_per_rule_group: yup.number().integer().meta({
    description:
      "Maximum number of rules per rule group for this tenant. 0 to disable."
  }),
  ruler_max_rule_groups_per_tenant: yup.number().integer().meta({
    description: "Maximum number of rule groups for this tenant. 0 to disable."
  }),
  store_gateway_tenant_shard_size: yup.number().integer().meta({
    description:
