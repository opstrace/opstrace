...
import moduleSubscriptionManager, {
  moduleSubscriptionEventChannel,
  executeActionsChannel
} from "state/module/sagas/subscription";
import * as actions from "state/module/actions";

describe("moduleSubscriptionManager", () => {
  const mockChannel = eventChannel(() => () => {});

  const mockTask: Task = createMockTask();

  test("should subscribe for new actions", async () => {
    const saga = moduleSubscriptionManager();
