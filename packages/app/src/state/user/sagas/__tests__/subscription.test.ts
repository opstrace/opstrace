...
import currentUserSubscriptionManager, {
  currentUserSubscriptionEventChannel,
  executeActionsChannel
} from "state/user/sagas/subscription";
import * as actions from "state/user/actions";

describe("currentUserSubscriptionManager", () => {
  const mockChannel = eventChannel(() => () => {});

  const mockTask: Task = createMockTask();

  test("should subscribe for new actions", async () => {
    const saga = currentUserSubscriptionManager();
  ...
  test("should not subscribe for actions twice", async () => {
    ...
    await expectSaga(subscribeSaga)
      .provide([
        // Mock part
        [
          matchers.call.fn(currentUserSubscriptionEventChannel),
          eventChannel(() => () => {})
        ]
      ])
      .take(actions.subscribe)
