...
import {
  ...
  SubscribeToCurrentUserDocument
...
/**
 * currentUserSubscriptionManager listens for subscribe and unsubscribe requests.
 *
 * There can only exist a single subscription at any given time.
 */
export default function* currentUserSubscriptionManager() {
 ...
 */
export function currentUserSubscriptionEventChannel(): EventChannel<Actions> {
  return eventChannel(emitter => {
    const subscription = subscriptionClient
      .subscribe<SubscribeToCurrentUserSubscription>({
        query: SubscribeToCurrentUserDocument
      })
      .subscribe({
        next: res => {
          if (res.data?.user && res.data.user.length > 0) {
            emitter(actions.setCurrentUser(res.data?.user[0]));
          }
        }
      });
