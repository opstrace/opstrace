...
And by the way, as far as cost goes, running the Opstrace cluster for an hour should cost no more than \$1.
...
```bash
curl -L https://go.opstrace.com/cli-latest-macos-tbz | tar xjf -
# test it
./opstrace --help
```
...
cat <<EOF > prometheus.yml
  remote_write:
    - url: "https://cortex-external.myteam.$OPSTRACE_CLUSTER_NAME.opstrace.io:8443/api/v1/push"
...
<match *>
  @type loki
  url https://loki-external.myteam.$OPSTRACE_CLUSTER_NAME.opstrace.io:8443
  flush_interval 5s
  bearer_token_file /var/run/tenant/token
