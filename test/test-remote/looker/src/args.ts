...
export function parseCmdlineArgs(): void {
  ...
  parser.add_argument("--n-fragments-per-push-message", {
    help:
      "Number of series fragments to serialize into a single binary push message " +
      "(HTTP POST request body), mixed from different series. Default: 1.",
    type: "int",
    metavar: "N",
    default: 1
  });
  ...
  parser.add_argument("--n-chars-per-msg", {
    help:
      "Number of characters per log message (ignored in metrics mode). " +
      "Default: 100.",
    type: "int",
    metavar: "N",
    default: 100
  ...
  parser.add_argument("--log-start-time", {
    help:
      ...
      "log time series. Does not apply in metrics mode. Is useful when " +
  ...
  parser.add_argument("--log-time-increment-ns", {
    help:
      ...
      "log series (between log entry timestamps) (ignored in metrics mode). " +
      "Default: 1 ns.",
    type: "int",
    metavar: "N",
    default: 1
  ...
  parser.add_argument("--metrics-time-increment-ms", {
    help:
      "Time difference in milliseconds between adjacent samples in a " +
      "time series. Default: 1 ms.",
    type: "int",
    metavar: "N",
    default: 1
  });
  ...
  readgroup.add_argument("--skip-read", {
    help:
      "Skip the readout in the write/read cycle, proceed to the next " +
      "cycle instead.",
    action: "store_true",
    default: false
  });
  ...
  parser.add_argument("--http-server-port", {
    help:
      "HTTP server listen port (serves /metrics Prometheus endpoint). " +
      "Default: try 8900-8990.",
    type: "int",
    default: 0
  });
  ...
  parser.add_argument("--cycle-stop-write-after-n-seconds-jitter", {
    help:
      "Add random number of seconds from interval [-J,J] " +
      "to --cycle-stop-write-after-n-seconds. Default: 0.",
    metavar: "J",
    type: "float",
    default: 0
  });
  ...
  parser.add_argument("--retry-post-deadline-seconds", {
    help: "Maximum time spent retrying POST requests, in seconds. Default: 420.",
    type: "int",
    default: 420
  });

  parser.add_argument("--retry-post-min-delay-seconds", {
    help: "Minimal delay between POST request retries, in seconds. Default: 3.",
    type: "int",
    default: 3
  });

  parser.add_argument("--retry-post-max-delay-seconds", {
    help: "Maximum delay between POST request retries, in seconds. Deault: 30.",
    type: "int",
    default: 30
  });
  ...
  parser.add_argument("--retry-post-jitter", {
    help:
      "Relative jitter to apply for calculating the retry delay " +
      "(1: max). Default: 0.5.",
    type: "float",
    metavar: "FLOAT",
    default: 0.5
  ...
  parser.add_argument("--invocation-id", {
    help:
      ...
      "If read validation is enabled then care should be taken to ensure " +
      "uniqueness at the server.",
    type: "str",
    default: ""
  });
