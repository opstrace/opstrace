...
suite("Loki API test suite", function () {
  ...
  test("debugtest", async function () {
    // @ts-ignore: TS2532: Object is possibly 'undefined'.
    const testname = testName(this);

    // Specify details of log record to be inserted.
    const sampleTimeRFC3339nano = "2021-07-05T10:10:01.123456789Z";
    ...
    const url =
      "https://loki.badteam.jp-devcluster2.opstrace.io/loki/api/v1/push";
    const headers: Record<string, string> = {
      "Content-Type": "application/x-protobuf"
    };

    const authtoken = process.env.FUNTOKEN;
    ...
    const result = await waitForLokiQueryResult(
      "https://loki.badteam.jp-devcluster2.opstrace.io",
      queryParams,
      1,
      true,
      1,
