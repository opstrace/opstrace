...
import { log, queryJSONAPI, waitForQueryResult } from "./index";
...
export async function waitForPrometheusTarget(
  ...
  try {
    ...
    log.info(
      `Waiting for target with tenant=${tenant} job=${jobLabel} via port-forward: ${url}`
    );
    await waitForQueryResult(
      ...
      data => {
        ...
        const filtered = targets.filter(
          target => target["labels"]["job"] === jobLabel
        );
        ...
        log.info(
          ...
          filtered
            .map(
              t =>
                `${t["labels"]["job"]}:${t["labels"]["namespace"]}/${t["labels"]["pod"]}`
            )
            .sort()
        );
        return filtered;
