...
build-and-push-controller-image:
	docker build . -f containers/controller/Dockerfile -t opstrace/controller:$(CHECKOUT_VERSION_STRING)
	echo "Size of docker image:"
	docker images --format "{{.Size}}" opstrace/controller:$(CHECKOUT_VERSION_STRING)
	docker push opstrace/controller:$(CHECKOUT_VERSION_STRING)


.PHONY: build-and-push-app-image
build-and-push-app-image:
	docker build . -f containers/app/Dockerfile -t opstrace/app:$(CHECKOUT_VERSION_STRING)
	echo "Size of docker image:"
	docker images --format "{{.Size}}" opstrace/app:$(CHECKOUT_VERSION_STRING)
	docker push opstrace/app:$(CHECKOUT_VERSION_STRING)
...
cli-set-build-info-constants:
	...
	sed -i.bak "s|CONTROLLER_IMAGE_DEFAULT_REPLACE_DURING_BUILD|opstrace/controller:${CHECKOUT_VERSION_STRING}|g" \
	        packages/config/src/clusterconfig.ts && rm packages/config/src/clusterconfig.ts.bak
	git --no-pager diff packages/config/src/clusterconfig.ts || exit 0
